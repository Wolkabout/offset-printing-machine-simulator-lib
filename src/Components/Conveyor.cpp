...
void Conveyor::setRatePerHour(int value) {
    ...
    std::shared_ptr<ConveyorRateMessage> message = std::make_shared<ConveyorRateMessage>(m_ratePerHour);
...
Conveyor::Conveyor(const std::string &name, ComponentMessageReceiver &machine, int maxRatePerHour,
    ...
    if (maxRatePerHour < m_minRatePerHour) {
        throw std::invalid_argument(
                "You can\'t put a rate lower than the minimut rate per hour! (" + std::to_string(m_minRatePerHour) + ')');
    }
...
//                    m_logger.Log(((Component &) *component.get()).getName() + " is not a tempo component!");
                }
            }
            m_runningLoop = true;
