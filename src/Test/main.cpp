...
void printActionResults(Logger &logger, ActionStatusMessage &message) {
    logger.Log("[ACM] " + std::to_string(static_cast<double>(message.getType())) + " | " + message.getContent());
}
...
int main() {
    try {
        ...
        std::shared_ptr<Feeder> feeder = std::make_shared<Feeder>("Feeder Test 1", *machine.get(),
                17000, 100);
        ...
        std::shared_ptr<PaintStation> cyan = std::make_shared<PaintStation>("PaintStation Cyan Test 1",
                *machine.get(), 10000, 10000);
        machine->addComponent(cyan);

        std::shared_ptr<PaintStation> magenta = std::make_shared<PaintStation>("PaintStation Magenta Test 1",
                *machine.get(), 10000, 10000);
        machine->addComponent(magenta);

        std::shared_ptr<PaintStation> yellow = std::make_shared<PaintStation>("PaintStation Yellow Test 1",
                *machine.get(), 10000, 10000);
        machine->addComponent(yellow);

        std::shared_ptr<PaintStation> black = std::make_shared<PaintStation>("PaintStation Black Test 1",
                *machine.get(), 10000, 10000);
        machine->addComponent(black);

        std::shared_ptr<Delivery> delivery = std::make_shared<Delivery>("Delivery Test 1",
                *machine.get(), 17000, 16950);
        machine->addComponent(delivery);

        std::shared_ptr<Conveyor> conveyor = std::make_shared<Conveyor>("Conveyor Test 1",
                *machine.get(), 15000, 15000);
